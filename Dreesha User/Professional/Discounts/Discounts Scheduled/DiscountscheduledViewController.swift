//
//  DiscountscheduledViewController.swift
//  Dreeshah
//
...
class DiscountscheduledViewController: UIViewController ,UITableViewDelegate,UITableViewDataSource{
    //Mark Outlets
    @IBOutlet weak var DiscountSceduledTableView: UITableView!
    ...
    var ArrButtonLabel = ["Scheduled","Scheduled","Scheduled","Scheduled"]
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        DiscountSceduledTableView.delegate=self
        DiscountSceduledTableView.dataSource=self
        DiscountSceduledTableView.reloadData()
    }
    ...
    @IBAction func CreateDiscountAct(_ sender: UIButton) {
...
extension DiscountscheduledViewController
{
        ...
        func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as! DiscountsScheuledTableViewCell
...
extension DiscountscheduledViewController{
    func DiscountsList(){
        ...
        viewModel.apiDiscountsList(Status: "scheduled"){(isSuccess , message) in
            Indicator.shared.hideProgressView()
            if isSuccess{
                self.isFetching = true
                tableCollectionErrors(view: self.DiscountSceduledTableView, text: "")
                self.DiscountSceduledTableView.reloadData()
            }else{
                if message == "not found"{
                    ...
                    if self.offset == 0{
                        self.viewModel.model = []
                        self.DiscountSceduledTableView.reloadData()
                        tableCollectionErrors(view: self.DiscountSceduledTableView, text: "No records found")
                    }else{
