//
//  DiscountsAllViewController.swift
//  Dreeshah
//
...
class DiscountsAllViewController: UIViewController ,UITableViewDelegate,UITableViewDataSource{
    //Mark Outlets
    @IBOutlet weak var DiscountAllTableView: UITableView!
    ...
    var ArrButtonLabel = ["Active" , "Expired" , "Expired","Scheduled"]
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        DiscountAllTableView.dataSource=self
        DiscountAllTableView.delegate=self
    }
    ...
    @IBAction func CreateDisCountAction(_ sender: UIButton) {
...
extension DiscountsAllViewController
{
        ...
        func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as! DiscountsAllTableViewCell
...
extension DiscountsAllViewController{
    func DiscountsList(){
        ...
        viewModel.apiDiscountsList(Status: "all"){(isSuccess , message) in
            Indicator.shared.hideProgressView()
            if isSuccess{
                self.isFetching = true
                tableCollectionErrors(view: self.DiscountAllTableView, text: "")
                self.DiscountAllTableView.reloadData()
            }else{
                if message == "not found"{
                    ...
                    if self.offset == 0{
                        self.viewModel.model = []
                        self.DiscountAllTableView.reloadData()
                        tableCollectionErrors(view: self.DiscountAllTableView, text: "No records found")
                    }else{
