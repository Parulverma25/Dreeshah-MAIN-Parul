//
//  DiscountsActiveViewController.swift
//  Dreeshah
//
...
class DiscountsActiveViewController: UIViewController,UITableViewDelegate,UITableViewDataSource
{
    //Mark Outlets
    @IBOutlet weak var DiscountActiveTableView: UITableView!
    ...
    var ArrButtonLabel = ["Active" , "Active","Active","Active"]
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        DiscountActiveTableView.delegate=self
        DiscountActiveTableView.dataSource=self
        DiscountActiveTableView.reloadData()
    }
...
extension DiscountsActiveViewController{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as! DiscountsActiveTableViewCell
...
extension DiscountsActiveViewController{
    func DiscountsList(){
        ...
        viewModel.apiDiscountsList(Status: "active"){(isSuccess , message) in
            Indicator.shared.hideProgressView()
            if isSuccess{
                self.isFetching = true
                tableCollectionErrors(view: self.DiscountActiveTableView, text: "")
                self.DiscountActiveTableView.reloadData()
            }else{
                if message == "not found"{
                    ...
                    if self.offset == 0{
                        self.viewModel.model = []
                        self.DiscountActiveTableView.reloadData()
                        tableCollectionErrors(view: self.DiscountActiveTableView, text: "No records found")
                    }else{
