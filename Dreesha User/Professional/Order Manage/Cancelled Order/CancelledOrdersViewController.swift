...
class CancelledOrdersViewController: UIViewController, UITableViewDataSource, UITableViewDelegate {
 ...
 //Mark Variables
    ...
    override func viewWillAppear(_ animated: Bool) {
        setBackWithTitle(titleVal: "Cancelled Orders")
        self.offset = 0
...
extension CancelledOrdersViewController {
    
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return viewModel.getOrderManageData()?.count ?? 0
    }
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as! CancelledOrdersTableViewCell
...
extension CancelledOrdersViewController {
    ...
    func hitApiOrderList(){
        ...
        viewModel.apiOrderList(Status: "cancelled", Limit: "10", Offset: String(self.offset)){(isSuccess , message) in
