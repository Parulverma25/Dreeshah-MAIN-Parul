//
//  BookPendingViewController.swift
//  Dreeshah
//
...
class BookPendingViewController: UIViewController,UITableViewDelegate ,UITableViewDataSource{
...
extension BookPendingViewController{
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return viewModel.getBookingListData()?.result.count ?? 0
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as! BookPendingTableViewCell
...
extension BookPendingViewController{
    ...
    func hitApiBookingList(){
        ...
        viewModel.apiProfessionalBookingList(limit: "10", offset: "0" , Status: "pending" ){(isSuccess , message) in
            Indicator.shared.hideProgressView()
            if isSuccess{
                if self.viewModel.getBookingListData()?.result.count == 0{
                    tableCollectionErrors(view: self.PendingTableView, text: "No results found")
                }else{
                    tableCollectionErrors(view: self.PendingTableView, text: "")
