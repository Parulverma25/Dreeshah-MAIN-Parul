//
//  BookCmpletedViewController.swift
//  Dreeshah
//
...
class BookCmpletedViewController: UIViewController,UITableViewDelegate ,UITableViewDataSource {
    //Mark Outlets
...
extension BookCmpletedViewController{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as! BookCompletedTableViewCell
...
extension BookCmpletedViewController{
    func hitApiBookingList(){
        ...
        viewModel.apiProfessionalBookingList(limit: "10", offset: "0" , Status: "completed" ){(isSuccess , message) in
            Indicator.shared.hideProgressView()
            if isSuccess{
                if self.viewModel.getBookingListData()?.result.count == 0{
                    tableCollectionErrors(view: self.CompletedTableView, text: "No results found")
                }else{
                    tableCollectionErrors(view: self.CompletedTableView, text: "")
