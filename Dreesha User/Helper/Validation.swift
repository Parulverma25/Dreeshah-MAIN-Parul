...
class Validation {
   ...
   public func validateAnyOtherTextField(otherField: String) -> Bool {
    ...
    public func validOnlyArabic(arabicTextField : String)->Bool{
//        [RegularExpression(@"^[\u0621-\u064A\u0660-\u0669]+$", ErrorMessage = "You can enter arabic characters only")]
        
        let arabicRegexString = "^[\u{0621}-\u{064A}\u{0660}-\u{0669}]+$"
        let trimmedString = arabicTextField.trimmingCharacters(in: .whitespaces)
        let validateOtherString = NSPredicate(format: "SELF MATCHES %@", arabicRegexString)
        let isValidateOtherString = validateOtherString.evaluate(with: trimmedString)
        return isValidateOtherString
    }
}
