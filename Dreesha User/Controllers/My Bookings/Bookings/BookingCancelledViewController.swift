...
class BookingCancelledViewController: UIViewController ,UITableViewDelegate , UITableViewDataSource{
    ...
    extension BookingCancelledViewController{
        func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
            return viewModel.getBookingListData()?.result.count ?? 0
        }
        
        func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
            let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as! BookingCancelTableViewCell
...
extension BookingCancelledViewController{
    func hitApiBookingsListSearch(Search : String , Filter : String){
        ...
        viewModel.apiBookingListfunc(limit: "10", offset: "0",Search: Search ,Filter: Filter, Status: "pending" ){(isSuccess , message) in
