//
//  QuotationReqApprovedViewController.swift
//  Dreesha User
//
...
class QuotationReqApprovedViewController: UIViewController {
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        hitApiQuotationsList(Search: searchvalStr, Filter: filtervalstr)
...
extension QuotationReqApprovedViewController: UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as! QuotationReqApprovedTableViewCell
      ...
      cell.requestStatusLbl.text =  viewModel.model[indexPath.row].status ?? ""
        ...
        cell.ImgView.sd_setImage(with: URL(string: imageurl), placeholderImage: #imageLiteral(resourceName: "Placeholding"))
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        if isFetching{
            self.offset += 10
            hitApiQuotationsList(Search: searchBar.text?.replacingOccurrences(of: " ", with: "") ?? "", Filter: "")
        }
    }

}
//MARK:- API Call
extension QuotationReqApprovedViewController{
    func hitApiQuotationsList(Search: String , Filter : String){
        
        Indicator.shared.showProgressView(self.parent?.view ?? self.view)
        
        viewModel.apiQuotationListfunc(limit: "10", offset: String(self.offset) , Status: "approved" ,Search: Search ,Filter: Filter ){(isSuccess , message) in
