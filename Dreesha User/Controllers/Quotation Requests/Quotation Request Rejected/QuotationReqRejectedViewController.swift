//
//  QuotationReqRejectedViewController.swift
//  Dreesha User
//
...
class QuotationReqRejectedViewController: UIViewController {
...
extension QuotationReqRejectedViewController: UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as! QuotationReqRejectedTableViewCell
      ...
      cell.requestStatusLbl.text =  viewModel.model[indexPath.row].status ?? ""
        cell.rejectReason.text = "Reason: \(viewModel.model[indexPath.row].rejectReason ?? "")"
...
//MARK:- API Call
extension QuotationReqRejectedViewController{
    func hitApiQuotationsList(Search: String , Filter : String){
        
        Indicator.shared.showProgressView(self.parent?.view ?? self.view)
        
        viewModel.apiQuotationListfunc(limit: "10", offset: String(self.offset) , Status: "rejected" ,Search: Search ,Filter: Filter ){(isSuccess , message) in
