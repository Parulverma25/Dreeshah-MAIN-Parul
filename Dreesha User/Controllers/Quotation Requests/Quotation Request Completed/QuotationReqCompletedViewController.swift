//
//  QuotationReqCompletedViewController.swift
//  Dreesha User
//
...
class QuotationReqCompletedViewController: UIViewController {
...
extension QuotationReqCompletedViewController: UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "cell") as! QuotationReqCompletedTableViewCell
...
//MARK:- API Call
extension QuotationReqCompletedViewController{
    func hitApiQuotationsList(Search: String , Filter : String){
        
        Indicator.shared.showProgressView(self.parent?.view ?? self.view)
        
        viewModel.apiQuotationListfunc(limit: "10", offset: String(self.offset) , Status: "completed" ,Search: Search ,Filter: Filter ){(isSuccess , message) in
            Indicator.shared.hideProgressView()
            if isSuccess{
                self.isFetching = true
                tableCollectionErrors(view: self.requestsTable, text: "")
